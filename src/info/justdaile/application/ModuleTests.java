	...
	public void start(Stage primaryStage) {
		...
		try {
			...
			this.optionMenu = new OptionMenu() {
							...
							default:
								option = new ButtonOption(optionText, valid){
									...
									public void selectionChanged(){
										...
										if(options == 1){
											optionMenu.onSubmitted(optionMenu.getOptions(EvaluatableOption::isSelected));
										}
									}
								};
								break;
				...
				@Override
				public void onSubmitted(EvaluatableOption...options) {					
					this.revealOptions();
					...
					if(options.length >= 0){
						...
						for(int i = 0; i < options.length; i++){
							EvaluatableOption option = options[i];
							answerText.append("\n-> " + option.toString());
						}
